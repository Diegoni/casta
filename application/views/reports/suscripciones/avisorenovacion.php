...
<?php
$titulo = $this->lang->line('Avís de renovació / Aviso de Renovación');
$borrador = FALSE;
$nIdDocumento =  $campana['cDescripcion']. '-' . $nIdCliente;
$dCreacion = time();
$cRefCliente = $campana['cDescripcion'];
...
<br />
<ul>
	<li>Client: <strong><?php echo $cliente['nIdCliente'];?></strong>
	</li>
	<li>Refèrencia: <strong><?php echo $cliente['cRandom'];?></strong></li>
</ul>
<br /><br />
També poden confirmar la seva subscripció a través de les següents
vies: <br /><br />
...
Els preus reflexats corresponen a l'últim periode facturat i poden
patir modificacions.<br /><br />
</p>
<hr/>
<p>Estimado/a: <strong><?php echo format_name($cliente['cNombre'], $cliente['cApellido'], $cliente['cEmpresa']);?></strong>
<br />
<br />
Le rogamos nos confirme si desea renovar las siguientes suscripciones, cercanas a su vencimiento, 
antes del día 1 de diciembre. Una vez superada esta fecha ALIBRI no se hace responsable de los posibles
retrasos en la recepción de los números.<br />
<br />
Tenga en cuenta que los editores no aceptan reclamaciones de ejemplares 
pasados tres meses de su publicación.
<br />
<br />
...
<a href="http://www.alibri.es/renovacion">http://www.alibri.es/renovacion</a> <br />
<br />
Utilice el siguiente código de cliente y
referencia que les será solicitado. No olvide hacer mención de su nombre
y apellido:<br />
<br />
<ul>
	<li>Cliente: <strong><?php echo $cliente['nIdCliente'];?></strong>
	</li>
	<li>Referencia: <strong><?php echo $cliente['cRandom'];?></strong></li>
</ul>
<br /><br />
También puede confirmar su suscripción por las siguientes formas:
<br /><br />
...
<table class="items">
	...
	<tr class="item-row">
		<td class="item-ct">
<?php echo format_enlace_cmd($linea['suscripcion']['nIdSuscripcion'], site_url('suscripciones/suscripcion/index/' . $linea['suscripcion']['nIdSuscripcion']));?>
		</td>
