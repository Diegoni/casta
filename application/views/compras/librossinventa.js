(function(){
    ...
    var store2 = new Ext.data.ArrayStore({
        fields: ['id', 'text'],
        data: [['cTitulo', _s('cTitulo')], ['cAutores', _s('cAutores')], ['s.cSeccion', _s('Sección')], ['s.cSeccion,cAutores', _s('Sección-Autor')], 
    ...
    var controls = [seccion, tipo, orden, 
    {
    	xtype:'checkbox', 
    	fieldLabel: _s('Nacionales'), 
    	name:'nacional', 
    	checked:true
    }];
    
    var url = site_url('compras/devolucion/libros_sin_venta');
    
    var form = Ext.app.formStandarForm({
        controls: controls,
        title: _s('Libros sin ventas por antiguedad'),
        url: url
    });
    seccion.store.load();
    form.show();
