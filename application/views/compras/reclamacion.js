(function(){
    try {
        ...
        if (title == '') 
            title = _s('Reclamación pedido proveedor');
        if (icon == '') 
            icon = 'iconoReclamacionPedidoProveedorTab';
        ...
        form.init({
            ...
            title: title,
            icon: icon,
            url: site_url('compras/reclamacion'),
            fn_load: fn_load,
			fn_lang: fn_lang,
            fn_save: fn_save,
            fn_reset: fn_reset,
        ...
        var fn_open = function(id){
         ...
         <?php echo 'var grid_search = ' . extjs_creategrid($modelo, $id.'_g_search', null, null, 'compras.reclamacion', $this->reg->get_id(), null, FALSE, null, 'mode:"search", fn_open: fn_open');?>;
        ...
        var fn_enviar = function(){
            documentosEnviar(form, _s('Enviar reclamación'), site_url('compras/reclamacion/send'));
        }
