...
class Solr extends MY_Controller
{
	 ...
	 * @return CLI
	 */
	function articulos($type = 'web', $file = false)
	{
		if (isset($_SERVER['REMOTE_ADDR'])) {
			die($this->lang->line('task-runner-cmd-only'));
		}
		...
		$dir = $this->config->item('bp.solr.path.articulo');
		...
		if ($q->num_rows() > 0)
		{
			...
			foreach ($q->result_array() as $row)
			{
				...
				if ($count >= MAX_ITEMS)
				{
					...
					if ($file)
					{
						file_put_contents("{$dir}cat.{$fcount}.xml", $xml);
						$fcount++;
					}
	...
	function articulos2($type = 'web', $file = false)
	{
		if (isset($_SERVER['REMOTE_ADDR'])) {
			die($this->lang->line('task-runner-cmd-only'));
		}
		...
		$this->db->flush_cache();
		$this->db->select('cTitulo, cAutores, cISBNBase, dbo.DTtoUnixTS(f.dCreacion) AS dCreacion')
		->select('cISBN, cISBN10, cISBNBase10, nEAN, nIdTipo, fPrecioOriginal, f.nIdEditorial, nIdColeccion, fPrecio')
		->select('ed.cNombre cEditorial')
		->from('Cat_Fondo f')
		->join('Cat_Editoriales ed', 'f.nIdEditorial = ed.nIdEditorial', 'left');
		#$q = $this->db->get();
		...
		if ($q->num_rows() > 0)
		{
			...
			foreach ($q->result_array() as $row)
			{
				...
				$id = $row['nIdLibro'];
				$this->_log("Leyendo {$id} - {$total}/" . $q->num_rows());
				$libro = $row; #$this->m_articulo->load(/*$v['id']*/ $id, TRUE);
