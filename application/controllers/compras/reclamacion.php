...
class Reclamacion extends MY_Controller
{
	...
	{
		parent::__construct('compras.reclamacion', 'compras/m_reclamacion', TRUE, 'compras/reclamacion.js', 'Reclamación pedido proveedor');
	}
	...
	protected function _post_get($id, $relations, &$data, $cmpid = null)
	{
		...
		$message = $this->load->view('compras/reclamacion', $data, TRUE);
		$this->load->library('HtmlFile');
		$filename = $this->obj->htmlfile->create($message, $this->lang->line('Reclamación pedido proveedor') . ' ' . $id, $this->config->item('bp.documentos.css'));
	...
	protected function _get_profile_sender($id)
	{
		...
		$subject = $this->lang->line('reclamacionpedidoproveedor-subject-email');
		$subject = str_replace('%id%', $id, $subject);
		return array(
			'perfil' 		=> array(PERFIL_RECLAMACIONES, PERFIL_PEDIDO),
			'emails'		=> $this->m_email,
			'faxes'			=> $this->m_telefono,
			'report_email' 	=> $this->config->item('sender.reclamacionpedidoproveedor'),
	...
	function crear($id = null)
	{
		...
		if ($id)
		{
			...
			if (count($pedidos) == 0) $this->out->error($this->lang->line('no-hay-lineas-por-reclamar'));
			...
			foreach($pedidos as $k => $v)
			{
				...
				if ($id_r < 0)
				{
					$this->messages->error(sprintf($this->lang->line('reclamacion-error'), $k, $this->reg->error_message()));
				}
				else
				{
					...
					if ($res['success'])
					{
						$link_r = format_enlace_cmd($id_r, site_url('compras/reclamacion/index/' . $id_r));
						...
						foreach($v['lineas'] as $l)
						{
							...
							$link_r = format_enlace_cmd($id_r, site_url('compras/reclamacion/index/' . $id_r));
							$message2 = sprintf($this->lang->line('reclamacion-titulo'), $l['nCantidad'], $link_l, $l['cTitulo'], $message);
			...
			$body = $this->messages->out($this->lang->line('Reclamar pedidos proveedor'));
			$this->out->html_file($body, $this->lang->line('Reclamar pedidos proveedor'), 'iconoReportTab');
		}
...
/* Location: ./system/application/controllers/compras/reclamacion.php */