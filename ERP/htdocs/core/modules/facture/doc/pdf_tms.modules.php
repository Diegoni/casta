 ...
 */
class pdf_tms extends ModelePDFFactures
{
    var $db;
    var $name;
    var $description;
	...
	function liquida_iva($ref)
	{
		global $conf;
		
		$sql = 
		"SELECT 
			`llx_societe`.`tva_assuj`, 
		...
		if($numr > 0)
		{
			$tva_array = $this->db->fetch_array($resql);
					
			if($tva_array['tva_assuj'] == 0 || $conf->global->FACTURE_TVAOPTION == 0)
			{
				return FALSE;
			}
			else
...
----------------------------------------------------------------------------*/
		...
		if($conf->global->FACTURE_TVAOPTION == 0)
		{
			$letra = 'C';
		}
		else
		...
		$pdf->SetXY(10 ,10);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 3, $letra, '', 'R');
		$pdf->Rect(97,10,16,14); // Cuadrado
...
/*----------------------------------------------------------------------------
		Buscar si la factura tiene CAE
----------------------------------------------------------------------------*/
		...
		FROM 
			`tms_cae`
		INNER JOIN 
			`llx_facture` ON(`tms_cae`.`id_facture` = `llx_facture`.`rowid`) 
		WHERE 
			`llx_facture`.`facnumber` = '$object->ref'";
		...
		if($numr > 0)
		{
			$cae_array = $this->db->fetch_array($resql);
			
			$primera_linea = 'CAE '.$cae_array['cae'];
			$segunda_linea = 'Vencimiento '.date('d-m-Y', strtotime($cae_array['vencimiento']));
		}
		else
		{
			$primera_linea = '';
		...
		$pdf->SetXY(10,250); // Posición
		$pdf->SetFont('','B', $default_font_size); // Letra
		$pdf->MultiCell(60, 3, $primera_linea, 0, 'L', 0); // Cadena
		
		$pdf->SetXY(10,255); // Posición
		$pdf->SetFont('','', $default_font_size); // Letra
		$pdf->MultiCell(60, 3, $segunda_linea, 0, 'L', 0); // Cadena
		
		$pdf->Rect(10,250,190,9); // Cuadrado
