...
<div id="product-prices" class="panel product-tab">
	...
	<h3>{l s='Product price'} VIVA PERON</h3>
	...
	<div class="form-group">
		...
		<div class="col-lg-2">
			<div class="input-group">
				...
				<input maxlength="27" name="wholesale_price" id="wholesale_price" type="text" value="{{toolsConvertPrice price=$product->wholesale_price}|string_format:$priceDisplayPrecisionFormat}" onchange="this.value = this.value.replace(/,/g, '.');" readonly/>
			</div>
	...
	<div class="form-group">
		...
		<div class="col-lg-2">
			<div class="input-group">
				...
				<input size="11" maxlength="27" id="priceTE" name="price_displayed" type="text" value="{{toolsConvertPrice price=$product->price}|string_format:'%.6f'}" onchange="noComma('priceTE'); $('#priceTEReal').val(this.value);" onkeyup="$('#priceType').val('TE'); $('#priceTEReal').val(this.value.replace(/,/g, '.')); if (isArrowKey(event)) return; calcPriceTI();" readonly/>
			</div>
		</div>
	</div>
	<div class="form-group">
		...
		<div class="col-lg-8">
			...
			<div class="row">
				<div class="col-lg-6">
					<select onchange="javascript:calcPrice(); unitPriceWithTax('unit');" name="id_tax_rules_group" id="id_tax_rules_group" {if $tax_exclude_taxe_option}disabled="disabled"{/if} readonly>
	...
	<div class="form-group">
		<div class="col-lg-9 col-lg-offset-3">
			<div class="alert">
				...
				<input type="hidden" value="{$product->getIdTaxRulesGroup()}" name="id_tax_rules_group" readonly />
			</div>
		</div>
	</div>
	{/if}
	...
	<div class="form-group" {if !$country_display_tax_label || $tax_exclude_taxe_option}style="display:none;"{/if} >
		...
		<div class="input-group col-lg-2">
			<span class="input-group-addon">{$currency->prefix}{$currency->suffix}</span>
			<input id="priceType" name="priceType" type="hidden" value="TE" readonly/>
			<input id="priceTI" name="priceTI" type="text" value="" onchange="noComma('priceTI');" maxlength="27" onkeyup="$('#priceType').val('TI');if (isArrowKey(event)) return;  calcPriceTE();" readonly/>
		</div>
	...
	<div class="form-group">
		...
		<div class="col-lg-4">
			<div class="input-group">
				<span class="input-group-addon">{$currency->prefix}{$currency->suffix}</span>
				<input id="unit_price" name="unit_price" type="text" value="{$unit_price|string_format:'%.6f'}" maxlength="27" onkeyup="if (isArrowKey(event)) return ;this.value = this.value.replace(/,/g, '.'); unitPriceWithTax('unit');" readonly/>
				<span class="input-group-addon">{l s='per'}</span>
				<input id="unity" name="unity" type="text" value="{$product->unity|htmlentitiesUTF8}"  maxlength="10" onkeyup="if (isArrowKey(event)) return ;unitySecond();" onchange="unitySecond();" readonly/>
			</div>
		</div>
	</div>
