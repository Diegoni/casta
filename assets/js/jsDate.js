/*
 * Name: jsDate Desc: VBScript native Date functions emulated for Javascript
 * Author: Rob Eberhardt, Slingshot Solutions - http://slingfive.com/ Note: see
 * jsDate.txt for more info
 */
...
// arrays (1-based)
Date.MonthNames = Date.monthNames; /*
...
Date.WeekdayNames = Date.dayNames; /*
...
Date.CDate = function(p_Date) {
	...
	var strTry = p_Date.replace(/\-/g, '/').replace(/\./g, '/').replace(/ /g,
			'/'); // fix separators
	strTry = strTry.replace(/pm$/i, " pm").replace(/am$/i, " am"); // and
	// meridian
	// spacing
	...
	if (strTry.indexOf(":")) { // if appears to have time
		...
		var strTryDate = new Date().toDateString() + ' ' + p_Date; // pre-pend
		// current
		// date
...
Date.DateDiff = function(p_Interval, p_Date1, p_Date2, p_FirstDayOfWeek) {
	...
	p_FirstDayOfWeek = (isNaN(p_FirstDayOfWeek) || p_FirstDayOfWeek == 0)
			? vbSunday
	...
	var nMonths = dt2.getUTCMonth() - dt1.getUTCMonth()
			+ (nYears != 0 ? nYears * 12 : 0);
	var nQuarters = parseInt(nMonths / 3); // <<-- different than VBScript,
	// which watches rollover not
	// completion
...
Date.DatePart = function(p_Interval, p_Date, p_FirstDayOfWeek) {
	...
	switch (p_Interval.toLowerCase()) {
		...
		case "y" :
			return Date.DateDiff("y", "1/1/" + dtPart.getFullYear(), dtPart)
					+ 1; // day of year
		case "d" :
		...
		case "ww" :
			return Date.DateDiff("ww", "1/1/" + dtPart.getFullYear(), dtPart,
					p_FirstDayOfWeek)
					+ 1; // week of year
		case "h" :
		...
		case "ms" :
			return dtPart.getMilliseconds(); // <-- JS extension, NOT in
			// VBScript
		default :
			return "invalid interval: '" + p_Interval + "'";
	}
...
Date.MonthName = function(p_Month, p_Abbreviate) {
	if (isNaN(p_Month)) { // v0.94- compat: extract real param from passed
		// date
...
Date.WeekdayName = function(p_Weekday, p_Abbreviate, p_FirstDayOfWeek) {
	if (isNaN(p_Weekday)) { // v0.94- compat: extract real param from passed
		// date
	...
	var nWeekdayNameIdx = ((p_FirstDayOfWeek - 1 + parseInt(p_Weekday) - 1 + 7) % 7)
			+ 1; // compensate nWeekdayNameIdx for p_FirstDayOfWeek
...
Date.Format = function(p_Date, p_Format, p_FirstDayOfWeek, p_firstweekofyear) {
	...
	var strShortTime = hr + ':' + this.pad(dt.getMinutes()) + ':'
			+ this.pad(dt.getSeconds()) + ' ' + ampm;
	var strShortDate = (dt.getMonth() + 1) + '/' + dt.getDate() + '/'
			+ new String(dt.getFullYear()).substring(2, 4);
	var strLongDate = Date.MonthName(dt.getMonth() + 1) + ' ' + dt.getDate()
			+ ', ' + dt.getFullYear(); //
	...
	retVal = retVal.replace(new RegExp('yy', 'gi'),
			new String(dt.getFullYear()).substring(2, 4));
	...
	retVal = retVal.replace(new RegExp('am/pm', 'g'), dt.getHours() >= 12
					? 'pm'
					: 'am');
	retVal = retVal.replace(new RegExp('AM/PM', 'g'), dt.getHours() >= 12
					? 'PM'
					: 'AM');
	retVal = retVal.replace(new RegExp('a/p', 'g'), dt.getHours() >= 12
					? 'p'
					: 'a');
	retVal = retVal.replace(new RegExp('A/P', 'g'), dt.getHours() >= 12
					? 'P'
					: 'A');
	retVal = retVal.replace(new RegExp('AMPM', 'g'), dt.getHours() >= 12
					? 'pm'
					: 'am');
	// (always proceed largest same-lettered token to smallest)

	// now finish the previously set-aside tokens
	retVal = retVal.replace(new RegExp('XXXX', 'gi'), Date.MonthName(dt
							.getMonth()
							+ 1, false)); //
	retVal = retVal.replace(new RegExp('XXX', 'gi'), Date.MonthName(dt
							.getMonth()
							+ 1, true)); //
	retVal = retVal.replace(new RegExp('AAAAAA', 'gi'), strLongDate);
	retVal = retVal.replace(new RegExp('AAAAA', 'gi'), strShortDate);
	retVal = retVal.replace(new RegExp('AAAA', 'gi'), Date.WeekdayName(dt
							.getDay()
							+ 1, false, p_FirstDayOfWeek)); // 
	retVal = retVal.replace(new RegExp('AAA', 'gi'), Date.WeekdayName(dt
							.getDay()
							+ 1, true, p_FirstDayOfWeek)); // 
	retVal = retVal.replace(new RegExp('TTTT', 'gi'), dt.getHours() + ':'
					+ this.pad(dt.getMinutes()));
	retVal = retVal.replace(new RegExp('TTT', 'gi'), hr + ':'
					+ this.pad(dt.getMinutes()) + ' ' + ampm);
	retVal = retVal.replace(new RegExp('CCCC', 'gi'), strShortDate + ' '
					+ strShortTime);

	// finally timezone
	tz = dt.getTimezoneOffset();
	timezone = (tz < 0) ? ('GMT-' + tz / 60) : (tz == 0)
			? ('GMT')
			: ('GMT+' + tz / 60);
...
/*
 * and other capitalizations for easier porting isDate = IsDate; dateAdd =
 * DateAdd; dateDiff = DateDiff; datePart = DatePart; monthName = MonthName;
 * weekdayName = WeekdayName; formatDateTime = FormatDateTime; format = Format;
 * 
 * isdate = IsDate; dateadd = DateAdd; datediff = DateDiff; datepart = DatePart;
 * monthname = MonthName; weekdayname = WeekdayName; formatdatetime =
 * FormatDateTime;
 * 
 * ISDATE = IsDate; DATEADD = DateAdd; DATEDIFF = DateDiff; DATEPART = DatePart;
 * MONTHNAME = MonthName; WEEKDAYNAME = WeekdayName; FORMATDATETIME =
 * FormatDateTime; FORMAT = Format;
 */
