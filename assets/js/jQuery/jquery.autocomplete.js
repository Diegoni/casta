jQuery.autocomplete = function(input, options) {
	...
	if (options.data != null) {
		...
		for (var i = 0; i < options.data.length; i++) {
			// if row is a string, make an array otherwise just reference
			// the array
			...
			if (row[0].length > 0) {
				...
				// if no lookup array for this character exists, look it up
				// now
	...
	function receiveData(q, data) {
		if (data) {
			...
			// if the field no longer has focus or if there are no matches,
			// do not display the drop down
