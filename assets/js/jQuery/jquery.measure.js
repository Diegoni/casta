...
(function($){
    ...
    $.extend($.fn.measure, {
        ...
        plugin: function(P, opts, fn){//P is for plugin
            if (this.length == 0) 
                return [];//abort plugin on empty selections
            var results = [];
            this.each(function(){
                var $el = $(this), page = this == document.body ? P.pageOptions : null, meta = $.metadata ? $el.metadata() : null,//metadata support
 o = $.extend(true, {}, P, page, meta, opts), ret = fn.call($el, o);
                if (ret === false) 
                    return ret;//abort each on false
                if (ret !== undefined) 
                    results.push(ret);
            });
            return results;
        ...
        pageBox: function(o){
            var d = document, b = d.body, full = $.boxModel && o.css.position != 'fixed', h = full ? $(d).height() : b.clientHeight, w = full ? $(d).width() : b.clientWidth;
            return {
                top: 0,
                left: 0,
                height: h,
