/*******************************************************************************
 * Ext.ux.TinyMCE v0.6 ExtJS form field containing TinyMCE v3.
 * 
 * Author: Andrew Mayorov (http://blogs.byte-force.com/xor) Copyright (c)2008
 * BYTE-force www.byte-force.com
 * 
...
(function() {
		...
		Ext.ux.TinyMCE.superclass.constructor.call(this, config);
			...
			// Members
			{
				...
				onRender : function(ct, position) {
					...
					this.ed.onChange.add(function(ed, l) {
						self.fireEvent("change");
							// alert('Editor contents was modified.');
						});

					// Set event handler on editor init.
					this.ed.onInit.add(function() {
								// Modify markup.
								var tbar = Ext.get(Ext.DomQuery.selectNode("#"
										+ this.ed.id + "_tbl td.mceToolbar"));
								var tbars = tbar.select("> table.mceToolbar");
								Ext.DomHelper.append(tbar, {
											tag : "div",
											style : {
												overflow : "hidden"
											}
										}, true).appendChild(tbars);
							}.createDelegate(this));

					// Bind to editor focus
					// this.ed.onActivate.add( this.focus.createDelegate( this,
					// [ false, false ], false ));
				...
				focus : function(selectText, delay) {
					Ext.ux.TinyMCE.superclass.focus.call(this, selectText,
							delay);
				},
				...
				/**
				 * ---------------------------------------------------------- If
				 * ed (local editor instance) is already initilized, calls
				 * specified function directly. Otherwise - adds it to ed.onInit
				 * event.
				 */
	...
	function(editor) {
				...
				open : function(s, p) {
					...
					if (!s.type)
						this.bookmark = this.editor.selection
								.getBookmark('simple');
				...
				setTitle : function(win, ti) {
					...
					// Probably not inline
					if (!win.tinyMCEPopup || !win.tinyMCEPopup.id) {
						// WindowManager.superclass.setTitle.call(this, win,
						// ti);
						return;
					}
		...
		ControlManager.superclass.constructor.call(this, ed, s);
			...
			// Members
			{
				...
				createDropMenu : function(id, s) {
					// Call base method
					var res = ControlManager.superclass.createDropMenu.call(
							this, id, s);
				...
				createColorSplitButton : function(id, s) {
					// Call base method
					var res = ControlManager.superclass.createColorSplitButton
							.call(this, id, s);
