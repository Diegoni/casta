...
e.stopEvent();m.showAt(e.getPoint());},createMenu:function(){if(!this.menu){var items=[{itemId:'close',iconCls:'icon-close',text:this.closeTabText,scope:this,handler:this.onClose}];if(this.showCloseAll){items.push('-');}
items.push({itemId:'closeothers',iconCls:'icon-closeothers',text:this.closeOtherTabsText,scope:this,handler:this.onCloseOthers});if(this.showCloseAll){items.push({itemId:'closeall',iconCls:'icon-closeall',text:this.closeAllTabsText,scope:this,handler:this.onCloseAll});}
