...
class SMStrend_SMS {
	...
	function SMStrend_SMS($username, $password) {
	...
	function validate() {
		...
		if (count($this->recipients) == 0) {
			...
			foreach ($this->recipients as $recipient) {
				if (!smstrend_is_valid_international($recipient)) {
					$this->problem = 'invalid recipient: '.$recipient;
					return false;
				}
			}
		}
		if (!smstrend_sms_type_valid($this->sms_type)) {
			$this->problem = 'invalid SMS type: $this->sms_type';
			return false;
		}
		if (smstrend_sms_type_has_custom_tpoa($this->sms_type)) {
			if (!smstrend_is_valid_tpoa($this->sender)) {
	...
	function set_scheduled_delivery($timestamp) {
		$this->scheduled_delivery = strftime(SMSTREND_DATE_TIME_FORMAT,$timestamp);
	}
	...
	function send() {
		...
		$post = new SMStrend_POST($this->username, $this->password);
		...
		$rp = $post->do_post(SMSTREND_SEND_SMS_REQUEST);
