...
function smstrend_is_valid_tpoa($tpoa) {
	return
	ereg('^00[0-9]{7,16}$',$tpoa) ||	// phone number in local format, or
	ereg('^\\+[0-9]{7,16}$',$tpoa) ||	// phone number in international format, or
	strlen($tpoa) < 12;			// < 12 chars alphanumeric string
}

function smstrend_is_valid_international($phone) {
	return ereg('^\\+[0-9]{7,16}$',$phone);
}

function smstrend_date_to_unix_timestamp($smstrend_date) {
	$res = strptime($smstrend_date,SMSTREND_DATE_TIME_FORMAT);
	if ($res != false)
	{
		#echo '<pre>'; echo $smstrend_date; var_dump($res); echo '</pre>';
